...
const PageIndex: React.FC = () => {
  ...
  return (
    <div className='flex flex-wrap gap-8'>
      {work && Object.entries(work).map(([code, workValue]) => (code === InitialBoardCode || workValue.comment) && (
        <Link className='card card-bordered card-compact w-72 shadow-xl' href={`/board/${code}`} key={code}>
          <figure>
            {pipe(
              IOBoard.decode(code),
              fold(
